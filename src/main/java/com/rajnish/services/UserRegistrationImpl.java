...
public class UserRegistrationImpl implements UserRegistration {
	...
	public String saveUser(User user) {
		...
		if(save !=null) {
		String mailBody = renderTemplate("EMAIL.txt", save);
		String to=user.getEmailId();
		String subject="Registration Successful";
		boolean sendEmail = sender.sendEmail(to, subject, mailBody);
		
        if(sendEmail) {
        	//System.out.println("Registration successfull! Please check your email to unlock your account!");
          message="Registration successfull! Please check your email to unlock your account!";
        }
        else {
         	//System.out.println("Email not recived! Contact to our team.");
        	  message="Email not recived! Contact to our team.";
              
        }
		}
		else 
		 //	System.out.println("Error occurs to save the data to database.");
		      message="Error occurs to save the data to database.";
...
//	}
	...
	public  String renderTemplate(String file,UserEntity user) {
		String mailBody=null;
		System.out.println(user+"  3333333333333333333333333333333333");
		try {
			FileReader fileReader=new FileReader("E:\\Ashok IT\\workspace\\01-registration-restapi\\src\\main\\resources\\EMAIL.txt");
			BufferedReader br=new BufferedReader(fileReader);
			
			StringBuffer renderBody=new StringBuffer();
			String data="";
			while((data=br.readLine())!=null) {
				renderBody.append(data);
			}
			String string = renderBody.toString();
			string=string.replace("{FNAME}", user.getFirstName());
			string=string.replace("{TEMP-PWD}", user.getPass());
			string=string.replace("{EMAIL}", user.getEmailId());
			mailBody=string;
			}catch(Exception ex) {
				ex.printStackTrace();
			}
